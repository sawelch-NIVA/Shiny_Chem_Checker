...
### Sidebar 1. General ----------------------------------------------------
        box(
            title = "1. Enter general information",
            width = NULL,
            status = "primary",
            solidHeader = TRUE,
            collapsible = TRUE,
            tags$div(class = "nav flex-column",
                     tags$a(href = "#general", "1.1 Enter general information", class = "nav-link")
            )
        ),
...
### Dashboard 1. General ------------------------------------------------------
        box(
            title = "1.1 Enter general information",
            width = NULL,
            status = "primary",
            solidHeader = TRUE,
            collapsible = TRUE,
...
### Dashboard 2. Sites ------------------------------------------------------
        box(
            ...
            tabsetPanel(
                tabPanel("Paste from Excel",
                         helpText("Paste your site data from Excel. The data should contain columns for site code, X coordinate, Y coordinate, and CRS."),
                         textAreaInput("bulkSites", 
                                       label = NULL, 
                                       rows = 8,
                                       placeholder = "Paste your data here..."),
                         actionButton("detectColumns", "Detect Columns"),
                         
                         # Column mapping (hidden initially)
                         div(id = "columnMapping",
                             style = "display: none;",
                             h4("Map your columns:"),
                             selectInput("siteCodeCol", "Site Code Column", choices = NULL),
                             selectInput("xCoordCol", "X Coordinate Column", choices = NULL),
                             selectInput("yCoordCol", "Y Coordinate Column", choices = NULL),
                             selectInput("crsCol", "CRS Column", choices = NULL)
                         )
                ),
                tabPanel("Manual Entry",
                         textInput("siteCode", "Site Code"),
                         numericInput("xCoord", "X Coordinate", value = NULL),
                         numericInput("yCoord", "Y Coordinate", value = NULL),
...
### Functions 2. Sites -----------------------------------------------------
    ...
    # Initialize reactive values for sites
    ...
    observeEvent(input$addSite, {
        ...
        tryCatch({
            # Create point and test transformation
            point <- st_point(c(input$xCoord, input$yCoord))
            point_sf <- st_sfc(point, crs = sub("EPSG:", "", input$crs))
            point_wgs84 <- st_transform(point_sf, 4326)
            
            # Get transformed coordinates
            coords <- st_coordinates(point_wgs84)
            
            # Basic boundary checks for transformed coordinates
            if (coords[1] < -180 || coords[1] > 180 || coords[2] < -90 || coords[2] > 90) {
                valid <- FALSE
                msg <- "Transformed coordinates are outside valid bounds"
            }
        }, error = function(e) {
            valid <- FALSE
            msg <- "Invalid coordinates for selected CRS"
        })
        ...
        showNotification("Site added successfully", type = "success")
    })
    
    # Remove sites handler
    observeEvent(input$removeSites, {
        ...
        if (length(input$sitesTable_rows_selected) > 0) {
            ...
            showNotification("Selected sites removed", type = "warning")
        }
    })
    ...
    observe({
        ...
        if (nrow(sites()) > 0) {
            ...
            sites_sf <- do.call(rbind, lapply(1:nrow(sites()), function(i) {
                site <- sites()[i,]
                point <- st_point(c(site$x, site$y))
                point_sf <- st_sfc(point, crs = sub("EPSG:", "", site$crs))
                point_wgs84 <- st_transform(point_sf, 4326)
                site_sf <- st_sf(
